...
from multi_mst import noisyMST, NoisyMST
...
def invariants(p, X):
    ...
    assert p.knn_neighbors_.shape == (X.shape[0], p.min_samples)
    assert p.knn_distances_.shape == (X.shape[0], p.min_samples)
    ...
    assert p.graph_neighbors_.shape[1] >= 1
    assert p.graph_distances_.shape[0] == X.shape[0]
    assert p.graph_distances_.shape[1] >= 1
...
def test_badargs(X):
    ...
    with pytest.raises(ValueError):
        noisyMST(X, num_neighbors=1.0)
    with pytest.raises(ValueError):
        noisyMST(X, num_neighbors=-1)
    with pytest.raises(ValueError):
        noisyMST(X, noise_fraction=1)
    with pytest.raises(ValueError):
        noisyMST(X, noise_fraction=-0.4)
    with pytest.raises(ValueError):
        noisyMST(X, min_samples=1.0)
    with pytest.raises(ValueError):
        noisyMST(X, num_neighbors=3, min_samples=4)
    with pytest.raises(ValueError):
        noisyMST(X, min_samples=0)
    with pytest.raises(ValueError):
        noisyMST(X, min_samples=-1)
...
def test_min_samples(X):
    ...
    p = NoisyMST(min_samples=3).fit(X)
    invariants(p, X)


def test_num_neighbors(X):
    """Tests with lower num_neighbors."""
    p = NoisyMST(num_neighbors=1).fit(X)
    invariants(p, X)


def test_noise_fraction(X):
    """Tests with epsilon."""
    p = NoisyMST(noise_fraction=0.5).fit(X)
    invariants(p, X)


def test_with_missing_data(X_missing):
    """Tests with nan data."""
    # cannot compare two noisy MSTs and expect them to be equal!
    model = NoisyMST().fit(X_missing)
...
def test_hdbscan_min_samples(X_missing):
    model = NoisyMST(min_samples=3).fit(X_missing)
    hdbscan = model.hdbscan(min_cluster_size=5)
