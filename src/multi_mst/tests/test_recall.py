...
def test_with_missing_data():
    """Tests with nan data."""
    model = KMSTDescentLogRecall().fit(X_missing_data)
    clean_model = KMSTDescentLogRecall().fit(X_missing_data[clean_indices])
...
def test_hdbscan_min_samples():
    model = KMSTDescentLogRecall(min_samples=3).fit(X_missing_data)
    hdbscan = model.hdbscan(min_cluster_size=5)
