...
from multi_mst.k_mst_descent import kMSTDescent, KMSTDescent
...
def test_badargs():
    ...
    assert_raises(ValueError, kMSTDescent, X, num_neighbors=1.0)
    assert_raises(ValueError, kMSTDescent, X, num_neighbors=-1)
    assert_raises(ValueError, kMSTDescent, X, min_descent_neighbors=1.0)
    assert_raises(ValueError, kMSTDescent, X, min_descent_neighbors=-1)
    assert_raises(ValueError, kMSTDescent, X, epsilon=1)
    assert_raises(ValueError, kMSTDescent, X, epsilon=0.6)
    assert_raises(ValueError, kMSTDescent, X, epsilon=-0.4)
    assert_raises(ValueError, kMSTDescent, X, min_samples=1.0)
    assert_raises(ValueError, kMSTDescent, X, num_neighbors=3, min_samples=4)
    assert_raises(ValueError, kMSTDescent, X, min_samples=0)
    assert_raises(ValueError, kMSTDescent, X, min_samples=-1)
...
def test_with_missing_data_graph_mode():
    """Tests with nan data."""
    clean_indices = list(range(1, 6)) + list(range(7, X.shape[0]))
    model = KMSTDescent(umap_kwargs=dict(transform_mode='graph')).fit(X_missing_data)
    clean_model = KMSTDescent(umap_kwargs=dict(transform_mode='graph')).fit(X_missing_data[clean_indices])
...
def test_graph_mode():
    ...
    p = KMSTDescent(umap_kwargs=dict(transform_mode='graph'))
    embedding = p.fit_transform(X)
...
def test_min_samples():
    ...
    p = KMSTDescent(min_samples=3, umap_kwargs=dict(transform_mode='graph')).fit(X)
...
def test_num_neighbors():
    ...
    p = KMSTDescent(num_neighbors=1, umap_kwargs=dict(transform_mode='graph')).fit(X)
...
def test_epsilon():
    """Tests with epsilon."""
    base = KMSTDescent(umap_kwargs=dict(transform_mode='graph')).fit(X)
    p = KMSTDescent(epsilon=1.1, umap_kwargs=dict(transform_mode='graph')).fit(X)
...
def test_num_components():
    ...
    p = KMSTDescent(umap_kwargs=dict(n_components=3))
