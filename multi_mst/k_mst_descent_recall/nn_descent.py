...
def initialize_out_graph(
    ...
    for i in numba.prange(data.shape[0]):
        ...
        # Some samples may be rejected, keep trying until enough samples are found.
        tries = 0
...
def nn_descent(
    ...
    true_selected_points = np.sort(true_selected_edges[1].flatten())
    true_selected_points = true_selected_points[np.argmax(true_selected_points >= 0) :]
    true_total_distance = _compute_dist_total(
        true_selected_points, true_neighbour_graph[0]
    )
    ...
    for i in range(n_iters):
        ...
        # Recall + distance fraction trace
        out_graph_copy = deheap_sort(out_graph[0].copy(), out_graph[1].copy())
        change_trace[i] = c
        recall_trace[i] = _compute_recall(true_neighbour_graph, out_graph_copy)
        dist_frac_trace[i] = (
...
def _compute_recall(truth, current):
    ...
    num_neighbours = truth[0].shape[1]
